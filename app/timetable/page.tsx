    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className="flex gap-1.5 flex-wrap">
          {COLOR_PALETTE.map((c) => (
            // key uses label (unique) not value (was causing duplicate key error)
            <button key={c.label} onClick={() => onChange({ ...value, color: c.value })} title={c.label}
            ...
            <div className="flex-1 overflow-y-auto p-2.5 space-y-2">
              ...
              {templates.length > 0 && (
                <div>
                  ...
                  {templates.map((tmpl) => (
                    <div key={tmpl.id}>
                      {editingId === tmpl.id ? (
                        <div className="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-3 mb-1.5">
                          <SessionForm
                            value={editForm} onChange={setEditForm}
                            onSubmit={() => handleSaveTemplateEdit(tmpl.id)}
                            onCancel={() => { setEditingId(null); setEditError(""); }}
                            submitLabel="Save" saving={editSaving} error={editError}
                          />
                        </div>
                      ) : (
                        <div draggable
                          onDragStart={() => {
                            dragPayload.current = {
                              type: "template", templateId: tmpl.id,
                              title: tmpl.title, duration: tmpl.duration,
                              color: tmpl.color, priority: tmpl.priority,
                              description: tmpl.description, offsetMinutes: 0,
                            };
                          }}
                          onDragEnd={() => { dragPayload.current = null; setGhostPos(null); }}
                          className="group relative rounded-xl p-2.5 mb-1.5 cursor-grab active:cursor-grabbing border border-neutral-100 dark:border-neutral-800 hover:border-neutral-200 dark:hover:border-neutral-700 bg-white dark:bg-neutral-900 transition-all select-none"
                          style={{ borderLeftColor: tmpl.color, borderLeftWidth: 3 }}>
                          <div className="flex items-start justify-between gap-1">
                            <p className="text-xs font-semibold text-neutral-800 dark:text-neutral-200 leading-tight line-clamp-2 flex-1">{tmpl.title}</p>
                            <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                              <button onClick={() => { setEditingId(tmpl.id); setEditForm({ title: tmpl.title, duration: tmpl.duration, priority: tmpl.priority, color: tmpl.color, description: tmpl.description }); setEditError(""); setSidebarMode(null); }}
                                className="p-0.5 rounded hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-300 hover:text-neutral-500 dark:hover:text-neutral-300 transition-colors">
                                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                              </button>
                              <button onClick={() => handleDeleteTemplate(tmpl.id)}
                                className="p-0.5 rounded hover:bg-red-50 dark:hover:bg-red-950 text-neutral-300 hover:text-red-400 transition-colors">
                                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                              </button>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 mt-1">
                            <span className="text-[10px] text-neutral-400">{formatDuration(tmpl.duration)}</span>
                            <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-medium ${tmpl.priority === "mandatory" ? "bg-red-50 dark:bg-red-950 text-red-400" : "bg-neutral-100 dark:bg-neutral-800 text-neutral-400"}`}>
                              {tmpl.priority}
                            </span>
                          </div>
                        </div>
              ...
              {templates.length === 0 && oneTimeSlots.length === 0 && sidebarMode === null && (
                <div className="py-8 text-center">
                  <p className="text-xs text-neutral-400 dark:text-neutral-600">No templates yet.</p>
                  <p className="text-xs text-neutral-300 dark:text-neutral-700 mt-1">Create one above and drag it onto the calendar.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="flex shrink-0 border-b border-neutral-100 dark:border-neutral-800 bg-white dark:bg-neutral-900">
              ...
              {DAYS.map((day, i) => {
                ...
                return (
                  <div key={day} className={`flex-1 text-center py-2.5 text-xs font-semibold ${isToday ? "text-emerald-500" : "text-neutral-500 dark:text-neutral-400"}`}>
                    <div>{day}</div>
                    <div className={`text-sm font-bold mt-0.5 ${isToday ? "w-6 h-6 rounded-full bg-emerald-500 text-white flex items-center justify-center mx-auto" : ""}`}>
                      {date.getDate()}
                    </div>
                  </div>
                );
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto">
              <div className="flex" style={{ height: DAY_HEIGHT }}>
                ...
                <div className="w-12 shrink-0 relative">
                  {hourLabels.map((m) => (
                    <div key={m} className="absolute right-2 text-[10px] text-neutral-400 dark:text-neutral-600 -translate-y-2"
                ...
                {DAYS.map((day, dayIndex) => {
                  ...
                  const isToday     = getDayDate(dayIndex).getTime() === today.getTime();
                  const now         = new Date();
                  const nowMin      = now.getHours() * 60 + now.getMinutes();
                  const showNowLine = isToday && nowMin >= GRID_START && nowMin <= GRID_END;

                  return (
                    <div key={day}
                      className={`flex-1 border-l border-neutral-100 dark:border-neutral-800 relative ${isToday ? "bg-emerald-500/[0.02]" : ""}`}
                      onDragOver={(e) => {
                        e.preventDefault();
                        const rect   = e.currentTarget.getBoundingClientRect();
                        const relY   = e.clientY - rect.top;
                        const minute = snapTo(GRID_START + relY / PX_PER_MIN, SNAP);
                        setGhostPos({ dayIndex, minute });
                      }}
                    ...
                    >
                      {/* Hour lines */}
                      {hourLabels.map((m) => (
                        <div key={m} className="absolute left-0 right-0 border-t border-neutral-100 dark:border-neutral-800"
                          style={{ top: (m - GRID_START) * PX_PER_MIN }} />
                      ))}
                      ...
                      {showNowLine && (
                        <div className="absolute left-0 right-0 z-20 pointer-events-none"
                          ...
                          <div className="h-px bg-emerald-500 opacity-60" />
                          <div className="absolute -left-1 -top-1 w-2 h-2 rounded-full bg-emerald-500 opacity-60" />
                        </div>
                      )}

                      {/* Ghost drop preview */}
                      {ghostPos?.dayIndex === dayIndex && dragPayload.current && (
                        <div className="absolute left-1 right-1 rounded-lg opacity-40 pointer-events-none z-10"
                          style={{
                            top: (ghostPos.minute - GRID_START) * PX_PER_MIN,
                            height: dragPayload.current.duration * PX_PER_MIN,
                            backgroundColor: dragPayload.current.color,
                          }} />
                      )}
                      ...
                      {daySessions.map((session) => {
                        ...
                        const height = Math.max(session.duration * PX_PER_MIN, 20);
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                      <span className="text-xs text-neutral-500 dark:text-neutral-400">
                        {DAYS[selected.dayIndex]} &middot; {formatTime(selected.startMinute)} &ndash; {formatTime(selected.startMinute + selected.duration)}
                      </span>
